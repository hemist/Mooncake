...
int initiator() {
    ...
    buffer_map[FLAGS_protocol].emplace_back(devAddr, g_TotalSize, "npu:" + std::to_string(g_devicePhyId));
    buffer_map[FLAGS_protocol].emplace_back(devAddr2, g_TotalSize, "npu:" + std::to_string(g_devicePhyId));
...
int target() {
    ...
    std::unordered_map<std::string, std::vector<mooncake::TransferEngine::RegisteredBuffer>> buffer_map;
    buffer_map[FLAGS_protocol].emplace_back(devAddr, g_TotalSize * FLAGS_recv_num, "npu:" + std::to_string(g_devicePhyId));
    buffer_map[FLAGS_protocol].emplace_back(devAddr2, g_TotalSize * FLAGS_recv_num, "npu:" + std::to_string(g_devicePhyId));
